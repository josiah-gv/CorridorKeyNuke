    ...
    try:
        ...
        def assign_outputs():
            ...
            for pass_name, pass_dir in output_passes.items():
                ...
                # Build a Nuke sequence path from the first file
                # e.g. "input.00130.exr" -> "input.%05d.exr" (or use ##### notation)
                first_file = out_files[0].replace('\\', '/')
                seq_path = _build_sequence_path(first_file)
                ...
                rn['file'].setValue(seq_path)
                rn['first'].setValue(frame_start)
                rn['last'].setValue(frame_end)
                rn['origfirst'].setValue(frame_start)
                rn['origlast'].setValue(frame_end)
